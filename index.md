...
For the design, I used a horizontal bar chart because bar charts are effective for comparing counts across discrete categories. In this plot, the x-axis encodes the number of sightings as a quantitative variable, while the y-axis encodes UFO shape as a nominal categorical variable. Color is also used to encode shape as a nominal variable. I used color to distinguish the different shape categories, which helps separate the bars visually and makes the ranking easier to scan. Because there are many possible shape values in the raw data, I restricted the display to the most common shapes so that the figure would remain readable and not become cluttered with rare categories.
